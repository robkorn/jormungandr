use crate::network::{
    service::ConnectionServices, ConnectionState, GlobalState, NetworkBlockConfig,
};
...
struct GrpcServer<B: NetworkBlockConfig> {
    state: ConnectionState<B>,
}

impl<B: NetworkBlockConfig> Clone for GrpcServer<B> {
    fn clone(&self) -> Self {
        GrpcServer {
